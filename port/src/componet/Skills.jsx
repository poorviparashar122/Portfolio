 
import Aos from 'aos';
import "../App.css"
import 'aos/dist/aos.css';
 export   const Skills = ()=>{

    return(
        <>
        <div  className="skills section" id="skills">
            <div className="skills-main">
            <h1 className="section-title"  style={{paddingTop:"10px"}}>Skills</h1>
            <div   style={{width:"70%", margin:"auto",display:"flex",justifyContent:"space-between"}}>

<div  className="skills-item" data-aos="fade-left">
    <img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/html_5.e7638e36.svg"/>
    <p>HTML</p>
</div>
<div  className="skills-item" data-aos="fade-left">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/css_3.e7a9de36.svg"/>
    <p>CSS</p>
</div>
<div  className="skills-item" data-aos="fade-left">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/javascript.fd46ca41.svg"/>
    <p>Javascript</p>
</div>
<div  className="skills-item" data-aos="fade-left">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/react.e27571ea.svg"/>
    <p>React</p>
</div>
<div  className="skills-item" data-aos="fade-left">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/redux.a9567540.svg"/>
    <p>Redux</p>
</div>
<div  className="skills-item" data-aos="fade-left">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/nodejs-icon.3dc364b8.svg"/>
    <p>Node.js</p>
</div>
</div>
<div   style={{width:"60%", margin:"auto",display:"flex",justifyContent:"space-between",marginTop:"30px"}}>

<div  className="skills-item" data-aos="fade-right">
    <img  className='skills-im' src="	https://portfolio-saravanakumarjn.vercel.app/static/media/express.7e85c513.svg"/>
    <p>ExpressJs</p>
</div>
<div  className="skills-item" data-aos="fade-right">
<img  className='skills-im' src="	https://portfolio-saravanakumarjn.vercel.app/static/media/mongodb.153b7b44.svg"/>
    <p>MongoDB</p>
</div>
<div  className="skills-item" data-aos="fade-right">
<img  className='skills-im' src="https://portfolio-saravanakumarjn.vercel.app/static/media/mysql.e5a509a5.svg"/>
    <p>MySQL</p>
</div>
<div  className="skills-item" data-aos="fade-right">
<img  className='skills-im' src="https://dev-hardik.netlify.app/assets/img/typescript.png"/>
    <p>TypeScript</p>
</div>
<div  className="skills-item" data-aos="fade-right">
<img  className='skills-im' src="data:image/png;base64,"/>
    <p>DS & ALgo</p>
</div>

</div>

            </div>
          


        </div>
        </>
    )

 }